...
class SkuSpider(BaseSpider):
    ...
    def get_jiage_page_url(self) -> list:
        ...
        response = requests.get(home_url, timeout=10, headers=headers)
    ...
    def get_tpey_details(self,type_url: str):
        ...
        for page_index in range(1, total_num+1):
            if(page_index == 1):
                ...
                headers = create_headers()
                response = requests.get(type_url[0:-5]+'p{0}.aspx'.format(page_index), timeout=10, headers=headers)
                html = response.content
                soup = BeautifulSoup(html, "lxml")
                title_div = soup.find_all('div', class_='title text-oneline')
                for title in title_div:
    ...
    def pack_sku_info(p_elements : list) -> SpuItem:
        content_str = ','
